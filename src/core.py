...
from src.mcp import fetch_weather, calculate_bmi
import pprint
        ...
        try:
            ...
            if isinstance(function_args, str):
                try:
                    ...
                    # 处理特殊的kwargs格式
                    if "kwargs" in function_args_dict:
                        kwargs_str = function_args_dict["kwargs"]
                        # 解析kwargs字符串，格式如 "city=东莞"
                        kwargs_dict = {}
                        if "=" in kwargs_str:
                        ...
                        function_args_dict = kwargs_dict
                except json.JSONDecodeError:
                    # 如果不是JSON，尝试解析为键值对
                    function_args_dict = {}
                    if "=" in function_args:
                        for pair in function_args.split(","):
                            if "=" in pair:
                                key, value = pair.split("=", 1)
            ...
            # 调用函数
            function_call_content = eval(f"{function_name}(**{function_args_dict})")
        except Exception as e:
            function_call_content = f"工具调用失败: {str(e)}"
